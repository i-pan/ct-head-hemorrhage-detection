"""
MedNeXt segmentation wrapper.

This wrapper exposes MedNeXt as a regular SKP model: it handles input
normalization, optional deep supervision, full-model weight loading, and
encoder/decoder freezing by MedNeXt module names.
"""
...
class Net(nn.Module):
    def __init__(self, cfg: Config):
        ...
        if self.cfg.get("load_pretrained_encoder") or self.cfg.get(
            "load_pretrained_decoder"
        ):
            raise ValueError(
                "MedNeXt does not support separate encoder/decoder loading in this wrapper. "
                "Use cfg.load_pretrained_model for full-model weights."
            )
    ...
    def forward(
        ...
        x = self.normalize(x)
        logits = self.mednext(x)

        if self.cfg.get("deep_supervision", False):
            out = {
                "logits": logits[0],
            }
            out.update({f"logits_ds{i}": logits[i] for i in range(1, len(logits))})
        else:
            out = {"logits": logits}
    ...
    def load_pretrained_model(self) -> None:
        ...
        missing_keys, unexpected_keys = self.load_state_dict(
            weights, strict=self.cfg.get("load_weights_strict", False) or False
        )
        if missing_keys:
